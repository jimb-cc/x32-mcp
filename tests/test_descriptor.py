...
def test_loads_real_file_default_and_explicit_path(d):
    ...
    assert "X32" in repr(d) and "2101 nodes" in repr(d)
...
def test_counts(d):
    ...
    assert len(list(d.iter_params())) == sum(len(p) for p in d.params.values()) == 443
    assert len(d.node_sections()) == 88
    assert d.roots == {
...
def test_all_node_paths_count_and_shape(d, capsys):
    ...
    assert total == 2101 and total < 2500
    assert per_family == {
        "ch": 992, "auxin": 200, "fxrtn": 192, "bus": 304, "mtx": 84, "main": 38, "dca": 16,
        "headamp": 128, "fx": 20, "config": 19, "show": 102, "stat": 5, "prefs": 1,
    }
