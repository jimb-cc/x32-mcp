...
Baseline JSON is written to $RTASIM_REPORT_DIR (default: a pytest tmp directory,
...
DEFAULT_REPORT_DIR = None  # pytest tmp dir unless RTASIM_REPORT_DIR is set


def _report_dir(tmp_path_factory) -> Path:
    env = os.environ.get("RTASIM_REPORT_DIR")
    p = Path(env) if env else Path(tmp_path_factory.mktemp("rtasim"))
    try:
