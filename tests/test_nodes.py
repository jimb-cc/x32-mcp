...
def test_round_trip_every_node_section(d):
    """Render → parse for all 2101 concrete node paths, padded and single-spaced."""
    n = 0
    for j, node in enumerate(d.nodes()):
...
async def test_dump_with_stub_node_many(d):
    ...
    assert conn.calls and conn.calls[0][1] == 4 and len(conn.calls[0][0]) == 2101
    assert set(st.missing) == drop
    assert len(st.sections) == 2101 - len(drop)
