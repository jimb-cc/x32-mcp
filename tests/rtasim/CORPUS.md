# CFS² offline validation corpus — critic's edition (`rtasim`, worktree `wt/corpus-critic`)

Test support only (pure Python, stdlib + `x32mcp`), deterministic per seed. Everything lives under `tests/rtasim/` of the
worktree `…/scratchpad/worktrees/corpus-critic` (branch `wt/corpus-critic`, built on the corpus builder's `wt/corpus` @ 9dfa5b7).
Citations: **[L §x]** = loop-physics brief, **[A §x]** = analyser brief, both under `…/scratchpad/reports/physics-*/brief.md`;
...
WT=$SP/worktrees/corpus-critic
PY=/Users/jimb/code/x32-mcp/.venv/bin/python

# simulator + harness sanity (21 tests, ~8 s)
$SP/runtests.sh $WT tests/test_rtasim.py tests/test_detector_corpus.py -k "not baseline and not closed_loop"

# the four baselines of §6 (current detector / pre-M7, open / closed loop, seeds 1-3; ~55 s; JSON + baseline_tables.md)
cd $WT && PYTHONPATH=src:tests $PY -m rtasim.run_baseline [$SP/reports/corpus-critic]

# ad hoc: any subset, current detector, table on stdout
cd $WT && PYTHONPATH=src:tests $PY -m rtasim.harness [--closed] [--json=out.json] X7_plateaued_ring_under_music_from_t0 S1_bass_under_quiet_music

# pytest wrapper that writes baseline_current_detector*.json (RTASIM_REPORT_DIR overrides the output dir)
$SP/runtests.sh $WT tests/test_detector_corpus.py -s
```
