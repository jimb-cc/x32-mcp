...
| `/-prefs/rta/det` | enum **{PEAK, RMS} = int 0/1** — raw 0 = PEAK, raw 1 = RMS: the console's own `/node` label, toggled both ways and read back (Verification log 2026-09-25 item 5). The DOC and X32.c `PRdet[]` list RMS first; `device.yaml` followed them until 2026-09-25, so every "PEAK" the server wrote before then set RMS | detector | — |
